...
class Resize {
	...
	public function create($file, $path, $filename, $sizes) {
		...
		if (is_array($sizes)) {
			...
			foreach ($sizes as $size) {
				...
				$resized[] = $this->doResize($path . $filename, $size[3] . $filename, $size[4]);
			}
			// dd($resized);
		}
	 ...
	 * @return bool
	 */
	private function doResize($image, $save_path, $image_quality) {
		$image = $this->openImage($image);
		// dd($image);
		$this->width = imagesx($image);
		$this->height = imagesy($image);

		// Get optimal width and height - based on $option.
		$option_array = $this->getDimensions($this->new_width, $this->new_height, $this->option);
		...
		if ('crop' == $this->option || 'fit' == $this->option) {
	...
	private function openImage($file) {
		...
		// dd(@imagecreatefromjpeg($file_path));
		switch ($mime) {
		case 'image/pjpeg': // IE6
		case 'image/jpeg':$img = @imagecreatefromjpeg($file_path);
			break;
		case 'image/gif':$img = @imagecreatefromgif($file_path);
			break;
		case 'image/png':$img = @imagecreatefrompng($file_path);
			break;
		default:$img = false;
			break;
		}
	...
	private function getDimensions($new_width, $new_height, $option) {
		...
		case 'auto':
			...
			break;
		case 'fit':
			$option_array = $this->getSizeByFit($new_width, $new_height);
			$optimal_width = $option_array['optimal_width'];
			$optimal_height = $option_array['optimal_height'];
			break;
		case 'crop':
	 ...
	 * @return array
	 */
	private function getSizeByFit($new_width, $new_height) {
